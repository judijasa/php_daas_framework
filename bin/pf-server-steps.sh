...
# db-check (warn-only reachability), and — on `worker`-tagged hosts only — the
# cron-manifest install.
#
...
if [ "${IS_WORKER_HOST:-0}" = "1" ]; then
    if [ -z "${CRON_FILE:-}" ]; then
        echo "pf-deploy: this host carries the 'worker' tag but deploy.conf sets no CRON_FILE." >&2
        exit 1
    fi
