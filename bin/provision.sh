...
#   2. Create the permanent dirs (log dir, DB data dir, deploy parent),
#      owned by PROD_USER.
#   3. Initialize the MariaDB cluster data dir if missing/empty.
#
...
mkdir -p "$DEPLOY_LOG_DIR" "$DEPLOY_DB_DATA_DIR"
chown -R "$PROD_USER:$PROD_USER" "$DEPLOY_LOG_DIR" "$DEPLOY_DB_DATA_DIR"
...
if [ ! -d "$DEPLOY_DB_DATA_DIR" ] || [ -z "$(ls -A "$DEPLOY_DB_DATA_DIR" 2>/dev/null)" ]; then
