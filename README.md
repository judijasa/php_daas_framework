...
# 1. Create the `test` database + dev connection file (git-ignored)
ema init db test

# 2. Apply the schema packages
ema init tables demo-8C3A9E1F0D2B4C5D test
...
- **`Utils\Connectivity\Database`** — the runner injects `$conn`, a real PDO
  connection created by `Database::admin('test')` using the `[test]` section
  of `var/reuter.local.ini` and the `MYSQL_UNIX_PORT` socket.
...
`var/reuter.local.ini`, generated by `ema init db <name>`) — run it after
changing the mapping (dev: `init-local-env.sh` runs it automatically; prod: the
consumer's `bin/deploy/post-nix.sh` calls it with the `REUTER_INI` path).
